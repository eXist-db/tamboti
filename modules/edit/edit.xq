                        ...
                        then 'compact-b-periodical' 
                            ...
                            then 'compact-b-anthology'
                                ...
                                then 'compact-b-series'
                                    ...
                                    if ($type-request = ('book-review-latin', 'book-review-transliteration')) 
...
let $target-collection-display := replace($target-collection, '/db/resources/users/', '')   
    let $target-collection-display := replace($target-collection-display, '/db/resources/commons/', '')
        let $target-collection-display := 
            if ($target-collection-display = $user) 
            then 'resources/Home'
            else ('resources/', $target-collection-display)
...
let $create-new-from-template :=
   ...
   (
       ...
       let $template-path := concat($config:edit-app-root, '/instances/', $type-request, '.xml')
       let $template-doc := doc($template-path)
       let $new-record-file-name := concat($id, '.xml')
       (: store it in the right location :)
       let $stored := xmldb:store($temporary-collection, $new-record-file-name, $template-doc)
       ...
       let $scriptTypeOfResource := doc(concat($config:edit-app-root, "/code-tables/language-3-type-codes.xml"))/code-table/items/item[value = $languageOfResource]/data(scriptClassifier)
       let $scriptTypeOfCataloging := doc(concat($config:edit-app-root, "/code-tables/language-3-type-codes.xml"))/code-table/items/item[value = $languageOfCataloging]/data(scriptClassifier)
...
<div class="content">
    ...
    }, on the <strong>{$bottom-tab-label}</strong> tab, to be saved in <strong>{$target-collection-display}</strong>.
    </span>
